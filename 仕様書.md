# FlowPython 要件定義（初版ドラフト）

最終更新: 2025-08-23

本書は、Web ブラウザ上で Simulink 的な「ブロック（ノード）を線でつないで Python を実行」でき、裏で Jupyter Notebook/Kernel を活用して途中結果や実行状態を保持・再利用可能とするシステムの要件定義のドラフトです。以後、合意に応じて更新します。
...
- 可視化（Plot）
	- 入力：DataFrame
	- 出力：Figure（または UI 表示のみ）
	- 主パラメータ：kind（line/bar/scatter/hist 等）、x, y, hue（列名）
	- 備考：v1 は matplotlib で画像出力、将来 plotly
...
- Notebook エクスポート：将来要件（v1.1 以降、任意）。現在はバックグラウンド実行が主。
...
### 4.6 永続化/バージョン管理
- プロジェクト保存：
	- graph.json（フローメタ/ノード/エッジ/レイアウト/設定）
	- notebooks/ へエクスポートされた .ipynb（任意）
...
### 4.9 ノートブックビュー（線形化）
- キャンバス（グラフ）と並行して、ノードをトポロジカル順に線形化した「ノートブックビュー（セル一覧）」を提供。
...
- フロントエンド：TypeScript + React、キャンバスは React Flow もしくは同等ライブラリ、コードエディタは Monaco。
- バックエンド：
...
- 実行エンジン：
	- 各ノードを実行用コードに変換し、カーネルに順次送信。
	- 変数名スキーマ：node_<id>_out<index> などで衝突回避。
	- キャッシュはファイル/メモリ併用（pickle/parquet/画像）。
- ストレージ：
...
- REST
	...
	- POST /projects/{id}/sheets, GET /projects/{id}/sheets, DELETE /projects/{id}/sheets/{sheetId}
	- GET /account/plan（プラン/シート上限/利用中シート数）
	- GET /account/compute-profiles（選択可能な計算資源プロファイル）
	- PUT /account/compute-profile（basic/standard/pro/gpu 等に切替）
	- POST /graphs/{id}/run, POST /graphs/{id}/run/nodes, POST /graphs/{id}/cancel
	- POST /kernel/start, POST /kernel/restart, POST /kernel/interrupt
	- POST /packages/install, GET /packages/list
	- （将来）POST /graphs/{id}/export/ipynb（任意）
- WebSocket
	- WS /streams/{graphId}：実行状態・ログ・各ノードの iopub メッセージを配信
	- イベント例：nodeStarted, nodeOutput(display/stream), nodeCleared, nodeError, nodeFinished, graphFinished
	- バックプレッシャ対応：メッセージ間引き/集約、心拍（ping/pong）
